...
3. `app.json` の `appleTeamId: "XXXXXXXXXX"` を実際の値に変更
4. Xcode で App Group capability を追加（アプリ本体・ウィジェット拡張の両方）
5. `expo prebuild` → Xcode ビルド → 実機インストール
6. 実機でウィジェット動作確認（ホーム画面・ロック画面）
